...
The dashboard is a data visualizer integrated into gaxis, intended to collect and visualize useful information of an Ethereum node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-axis` repo), these need to be installed first:
...
```
$ gaxis --dashboard --vmodule=dashboard=5
