...
// Package utils contains internal helper functions for go-axis commands.
package utils
...
import (
	...
	"github.com/axis-cash/go-axis/accounts/keystore"
	"github.com/axis-cash/go-axis/common"
	"github.com/axis-cash/go-axis/common/fdlimit"
	...
	"github.com/axis-cash/go-axis/metrics/influxdb"
	"github.com/axis-cash/go-axis/node"
	"github.com/axis-cash/go-axis/p2p"
	"github.com/axis-cash/go-axis/p2p/discover"
	"github.com/axis-cash/go-axis/p2p/discv5"
	"github.com/axis-cash/go-axis/p2p/nat"
	"github.com/axis-cash/go-axis/p2p/netutil"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		...
		Value: axis.DefaultConfig.NetworkId,
	}
	...
	OfflineFlag = cli.BoolFlag{
		Name:  "offline",
		Usage: "Use gaxis as the offline mode",
	}
	...
	defaultSyncMode = axis.DefaultConfig.SyncMode
	...
	EthashCachesInMemoryFlag = cli.IntFlag{
		...
		Value: axis.DefaultConfig.Ethash.CachesInMem,
	}
	EthashCachesOnDiskFlag = cli.IntFlag{
		Name:  "ethash.cachesondisk",
		Usage: "Number of recent ethash caches to keep on disk (16MB each)",
		Value: axis.DefaultConfig.Ethash.CachesOnDisk,
	}
	EthashDatasetDirFlag = DirectoryFlag{
		Name:  "ethash.dagdir",
		Usage: "Directory to store the ethash mining DAGs (default = inside home folder)",
		Value: DirectoryString{axis.DefaultConfig.Ethash.DatasetDir},
	}
	EthashDatasetsInMemoryFlag = cli.IntFlag{
		Name:  "ethash.dagsinmem",
		Usage: "Number of recent ethash mining DAGs to keep in memory (1+GB each)",
		Value: axis.DefaultConfig.Ethash.DatasetsInMem,
	}
	EthashDatasetsOnDiskFlag = cli.IntFlag{
		Name:  "ethash.dagsondisk",
		Usage: "Number of recent ethash mining DAGs to keep on disk (1+GB each)",
		Value: axis.DefaultConfig.Ethash.DatasetsOnDisk,
	}
	...
	TxPoolPriceLimitFlag = cli.Uint64Flag{
		...
		Value: axis.DefaultConfig.TxPool.PriceLimit,
	}
	TxPoolAccountSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.accountslots",
		Usage: "Minimum number of executable transaction slots guaranteed per account",
		Value: axis.DefaultConfig.TxPool.AccountSlots,
	}
	TxPoolGlobalSlotsFlag = cli.Uint64Flag{
		Name:  "txpool.globalslots",
		Usage: "Maximum number of executable transaction slots for all accounts",
		Value: axis.DefaultConfig.TxPool.GlobalSlots,
	}
	TxPoolAccountQueueFlag = cli.Uint64Flag{
		Name:  "txpool.accountqueue",
		Usage: "Maximum number of non-executable transaction slots permitted per account",
		Value: axis.DefaultConfig.TxPool.AccountQueue,
	}
	TxPoolGlobalQueueFlag = cli.Uint64Flag{
		Name:  "txpool.globalqueue",
		Usage: "Maximum number of non-executable transaction slots for all accounts",
		Value: axis.DefaultConfig.TxPool.GlobalQueue,
	}
	TxPoolLifetimeFlag = cli.DurationFlag{
		Name:  "txpool.lifetime",
		Usage: "Maximum amount of time non-executable transaction are queued",
		Value: axis.DefaultConfig.TxPool.Lifetime,
	}
	...
	GasPriceFlag = BigFlag{
		...
		Value: axis.DefaultConfig.GasPrice,
	}
	...
	AxisStatsURLFlag = cli.StringFlag{
		Name:  "axisstats",
		Usage: "Reporting URL of a axisstats service (nodename:secret@host:port)",
	}
	...
	GpoBlocksFlag = cli.IntFlag{
		...
		Value: axis.DefaultConfig.GPO.Blocks,
	}
	GpoPercentileFlag = cli.IntFlag{
		Name:  "gpopercentile",
		Usage: "Suggested gas price is the given percentile of a set of recent transaction gas prices",
		Value: axis.DefaultConfig.GPO.Percentile,
	}
	// Metrics flags
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("-------------------------------------------------------------------")
	log.Warn("Referring to accounts by order in the keystore folder is dangaxisus!")
	log.Warn("This functionality is deprecated and will be removed in the future!")
	log.Warn("Please use explicit addresses! (can search via `gaxis account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "AXIS", cfg.MaxPeers)
	...
	if ctx.GlobalIsSet(ConfirmedBlockFlag.Name) {
		...
		if balanceDelay >= 0 {
			axisparam.InitComfirmedBlock(balanceDelay)
		}
	}
	...
	if ctx.GlobalIsSet(ResetBlockNumber.Name) {
		...
		axisparam.InitCurrentBlockNumber(blockNumber)
	}
	...
	if ctx.GlobalBool(DeveloperFlag.Name) {
		...
		axisparam.Init_Dev(true)
	}
	if ctx.GlobalBool(OfflineFlag.Name) {
		axisparam.Init_Offline(true)
	}
...
func setEthash(ctx *cli.Context, cfg *axis.Config) {
...
// SetAxisConfig applies axis-related command line flags to the config.
func SetAxisConfig(ctx *cli.Context, stack *node.Node, cfg *axis.Config) {
	...
	if ctx.GlobalIsSet(ExchangeValueStrFlag.Name) {
		axisparam.InitExchangeValueStr(true)
	}
...
// RegisterEthService adds an Axis client to the stack.
func RegisterEthService(stack *node.Node, cfg *axis.Config) {
	err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		fullNode, err := axis.New(ctx, cfg)
		return fullNode, err
	})
	if err != nil {
		Fatalf("Failed to register the Axis service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gaxis.", map[string]string{
				"host": hosttag,
			})
		}
	}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) axisdb.Database {
	var (
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb axisdb.Database) {
	var err error
	...
	if !ctx.GlobalBool(FakePoWFlag.Name) {
		engine = ethash.New(ethash.Config{
			CacheDir:       stack.ResolvePath(axis.DefaultConfig.Ethash.CacheDir),
			CachesInMem:    axis.DefaultConfig.Ethash.CachesInMem,
			CachesOnDisk:   axis.DefaultConfig.Ethash.CachesOnDisk,
			DatasetDir:     stack.ResolvePath(axis.DefaultConfig.Ethash.DatasetDir),
			DatasetsInMem:  axis.DefaultConfig.Ethash.DatasetsInMem,
			DatasetsOnDisk: axis.DefaultConfig.Ethash.DatasetsOnDisk,
		})
	}
	if gcmode := ctx.GlobalString(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	...
	cache := &core.CacheConfig{
		...
		TrieNodeLimit: axis.DefaultConfig.TrieCache,
		TrieTimeLimit: axis.DefaultConfig.TrieTimeout,
	}
	if ctx.GlobalIsSet(CacheFlag.Name) || ctx.GlobalIsSet(CacheGCFlag.Name) {
...
// new format.
//
// e.g. gaxis account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gaxis --keystore /tmp/mykeystore --lightkdf account new
//
