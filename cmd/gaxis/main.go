...
// gaxis is the official command-line client for Axis.
package main
...
import (
	...
	"github.com/axis-cash/go-axis/console"
	"github.com/axis-cash/go-axis/internal/debug"
...
const (
	clientIdentifier = "gaxis" // Client identifier to advertise over the network
)
...
var (
	...
	app = utils.NewApp(gitCommit, "the go-axis command line interface")
...
func init() {
	// Initialize the CLI app and start Axis
	app.Action = gaxis
...
// gaxis is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gaxis(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if axisparam.Is_Dev() && ctx.GlobalString(utils.DeveloperPasswordFlag.Name) != "" {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) {
		// Mining only makes sense if a full Axis node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		var axis *axis.Axis
		if err := stack.Service(&axis); err != nil {
			utils.Fatalf("Axis service not running: %v", err)
		}
		...
		if threads := ctx.GlobalInt(utils.MinerThreadsFlag.Name); threads > 0 {
			...
			if th, ok := axis.Engine().(threaded); ok {
				th.SetThreads(threads)
			}
		}
		// Set the gas price to the limits from the CLI and start mining
		axis.TxPool().SetGasPrice(utils.GlobalBig(ctx, utils.GasPriceFlag.Name))
		if err := axis.StartMining(true); err != nil {
