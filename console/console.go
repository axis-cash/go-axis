...
import (
	...
	"github.com/axis-cash/go-axis/internal/jsre"
	"github.com/axis-cash/go-axis/internal/web3ext"
...
func (c *Console) init(preload []string) error {
	...
	flatten := "var axis = web3.axis; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = axis.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		axis, err := c.jsre.Get("axis")
		if err != nil {
			return err
		}

		if obj := axis.Object(); obj != nil {
			if _, err = c.jsre.Run(`jeth.sendTransaction = axis.sendTransaction;`); err != nil {
				return fmt.Errorf("axis.sendTransaction: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.createPkg = axis.createPkg;`); err != nil {
				return fmt.Errorf("axis.createPkg: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.transferPkg = axis.transferPkg;`); err != nil {
				return fmt.Errorf("axis.transferPkg: %v", err)
			}
			if _, err = c.jsre.Run(`jeth.closePkg = axis.closePkg;`); err != nil {
				return fmt.Errorf("axis.closePkg: %v", err)
			}
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines axis.getBalance(axis.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
func (c *Console) Welcome() {
	...
	fmt.Fprintf(c.printer, "Welcome to the Gaxis JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + axis.coinbase);
		console.log("at block: " + axis.blockNumber + " (" + new Date(1000 * axis.getBlock(axis.blockNumber).timestamp) + ")");
	...
	if apis, err := c.client.SupportedModules(); err == nil {
		...
		for api, version := range apis {
			if api == "axis" {
				modules = append(modules, fmt.Sprintf("%s:%s", "axis", version))
			} else {
