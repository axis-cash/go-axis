...
func New(axis Backend, config *params.ChainConfig, mux *event.TypeMux, voter voter, engine consensus.Engine) *Miner {
	miner := &Miner{
		axis:     axis,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, accounts.Account{}, voter, axis, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(axis.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) SetAxisbase(account accounts.Account) {
	self.coinbase = account
	self.worker.setAxisbase(account)
}
