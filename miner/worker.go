...
func newWorker(config *params.ChainConfig, engine consensus.Engine, account accounts.Account, voter voter, axis Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:      config,
		engine:      engine,
		eth:         axis,
		...
		chainDb:     axis.ChainDb(),
		recv:        make(chan *Result, resultQueueSize),
		powRecv:     make(chan *Result),
		posTaskCh:   make(chan *Result),
		chain:       axis.BlockChain(),
		proc:        axis.BlockChain().Validator(),
		coinbase:    account,
		agents:      make(map[Agent]struct{}),
		unconfirmed: newUnconfirmedBlocks(axis.BlockChain(), miningLogAtDepth),
		voter:       voter,
		pendingVote: newPendingVote(),
	}
	// Subscribe NewTxsEvent for tx pool
	worker.txsSub = axis.TxPool().SubscribeNewTxsEvent(worker.txsCh)
	worker.voteSub = worker.voter.SubscribeWorkerVoteEvent(worker.voteCh)
	// Subscribe events for blockchain
	worker.chainHeadSub = axis.BlockChain().SubscribeChainHeadEvent(worker.chainHeadCh)
	worker.chainSideSub = axis.BlockChain().SubscribeChainSideEvent(worker.chainSideCh)
	go worker.update()
	go worker.voteLoop()
...
func (self *worker) setAxisbase(account accounts.Account) {
	self.mu.Lock()
