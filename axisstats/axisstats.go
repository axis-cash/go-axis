...
import (
	...
	//"github.com/axis-cash/go-axis/lesdeprecate"
...
// Service implements an Axis netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server // Peer-to-peer server to retrieve networking infos
	eth    *axis.Axis  // Full Axis service if monitoring a full node
	//lesdeprecate    *lesdeprecate.LightEthereum // Light Axis service if monitoring a light node
...
//func New(url string, ethServ *axis.Axis, lesServ *lesdeprecate.LightEthereum) (*Service, error) {
func New(url string, ethServ *axis.Axis) (*Service, error) {
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["axis"]; info != nil {
		network = fmt.Sprintf("%d", info.(*axis.NodeInfo).Network)
		protocol = fmt.Sprintf("axis/%d", axis.ProtocolVersions[0])
	} else {
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the axisstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to axisstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to axisstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to axisstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to axisstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to axisstats")
