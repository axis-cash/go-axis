...
import (
	...
	"github.com/axis-cash/go-axis/axis/fetcher"
...
// NewProtocolManager returns a new Axis sub protocol manager. The Axis sub protocol manages peers capable
...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkID uint64, mux *event.TypeMux, voter shareVoter, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb axisdb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < axis63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Axis peer", "peer", id)

	// Unregister the peer from the downloader and Axis peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Axis protocol")
	...
	log.Info("Axis protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an axis peer. When
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Axis peer connected", "name", p.Name())

	// Execute the Axis handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("Axis handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Axis peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Axis message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= axis63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= axis63 && msg.Code == NodeDataMsg:
	...
	case p.version >= axis63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= axis63 && msg.Code == ReceiptsMsg:
...
// NodeInfo represents a short summary of the Axis sub-protocol metadata
