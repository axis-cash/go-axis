...
import (
	...
	"github.com/axis-cash/go-axis/common/bitutil"
...
const (
	// bloomServiceThreads is the number of goroutines used globally by an Axis
...
func (eth *Axis) startBloomHandlers() {
...
// for the Axis header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  axisdb.Database      // database instance to write index data and metadata into
...
func NewBloomIndexer(db axisdb.Database, size uint64) *core.ChainIndexer {
	...
	table := axisdb.NewTable(db, string(rawdb.BloomBitsIndexPrefix))
