...
// PublicAxisAPI provides an API to access Axis full node-related
// information.
...
// PrivateAdminAPI is the collection of Axis full node-related APIs
...
// admin methods of the Axis service.
...
// PublicDebugAPI is the collection of Axis full node APIs exposed
...
// related public debug methods of the Axis service.
func NewPublicDebugAPI(eth *Axis) *PublicDebugAPI {
	return &PublicDebugAPI{axis: eth}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.axis.miner.Pending()
		return stateDb.RawDump(), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.axis.blockchain.CurrentBlock()
	} else {
		block = api.axis.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.axis.BlockChain().StateAt(block.Header())
	if err != nil {
...
// PrivateDebugAPI is the collection of Axis full node APIs exposed over
...
// private debug methods of the Axis service.
