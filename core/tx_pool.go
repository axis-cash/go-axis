...
func (pool *TxPool) checkDescCmd(tx *stx.T, state *state.StateDB) (err error) {
	...
	if cmd.BuyShare != nil {
		...
		if cmd.RegistPool.FeeRate > axisparam.HIGHEST_STAKING_NODE_FEE_RATE {
			err = fmt.Errorf("registPool Vote fee must <= %v%%", axisparam.HIGHEST_STAKING_NODE_FEE_RATE/100)
			return
		}
		if cmd.RegistPool.FeeRate < axisparam.LOWEST_STAKING_NODE_FEE_RATE {
			err = fmt.Errorf("registPool Vote fee must >= %v%%", axisparam.LOWEST_STAKING_NODE_FEE_RATE/100)
			return
		}
...
func (pool *TxPool) add(tx *types.Transaction, local bool) (bool, error) {
	...
	if true {
		if (axisparam.XIP6()-25) < currentBlockNum && currentBlockNum < (axisparam.XIP6()+25) {
			return false, fmt.Errorf("protect XIP6:%v", axisparam.XIP6())
		}
	}
