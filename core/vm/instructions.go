...
func handleAllotTicket(d []byte, evm *EVM, contract *Contract, mem []byte) (common.Hash, uint64, error, bool) {
	...
	if strings.Contains(categoryName, "AXIS") {
		return common.Hash{}, 0, fmt.Errorf("allotTicket error , contract : %s, error : %s", contract.Address(), "categoryName can not contains AXIS"), false
	}
...
func handleIssueToken(d []byte, evm *EVM, contract *Contract, mem []byte) (bool, error) {
	...
	if strings.Contains(coinName, "AXIS") {
		return false, fmt.Errorf("issueToken error , contract : %s, error : %s", contract.Address(), "coinName can not contains AXIS")
	}
	address := evm.StateDB.GetContrctAddressByToken(coinName)
	if address == (common.Address{}) {
		fee := new(big.Int).Set(level6)
		if evm.chainConfig.ChainID.Uint64() == 2020 {
			fee = tokenFee(coinName)
		}
		if evm.StateDB.GetBalance(contract.Address(), "AXIS").Cmp(fee) >= 0 {
			if !evm.StateDB.RegisterToken(contract.Address(), coinName) {
				...
				evm.StateDB.SubBalance(contract.Address(), "AXIS", fee)
				asset := assets.Asset{Tkn: &assets.Token{
					Currency: *common.BytesToHash(common.LeftPadBytes([]byte("AXIS"), 32)).HashToUint256(),
