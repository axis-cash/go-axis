...
import (
	...
	"github.com/axis-cash/go-axis/zero/txtool/verify"
	...
	"github.com/axis-cash/go-axis/common/mclock"
	...
	"github.com/axis-cash/go-axis/trie"
...
type BlockChain struct {
	...
	db     axisdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db axisdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, accountManager *accounts.Manager) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (self *State1BlockChain) GetDB() axisdb.Database {
	return self.Bc.db
}
...
func (self *State1BlockChain) IsValid() bool {
	...
	if header.Number.Uint64() < axisparam.DefaultConfirmedBlock() {
		return false
	}
...
func (self *State1BlockChain) GetAxisGasLimit(to *common.Address, tfee *assets.Token, gas *big.Int) (gaslimit uint64, e error) {
	if state, err := self.Bc.State(); err != nil {
		e = err
		return
	} else {
		return state.GetAxisGasLimit(to, tfee, gas)
	}
}
...
func (bc *BlockChain) GetDB() axisdb.Database {
	return bc.db
}
...
func (bc *BlockChain) loadLastState() error {
	...
	if axisparam.DefaultCurrentBlock() > 0 {
		currentBlock = bc.GetBlockByNumber(axisparam.DefaultCurrentBlock())
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= axisdb.IdealBatchSize {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	if block.Header().Number.Uint64() >= axisparam.XIP1() {
	...
	if bc.cacheConfig.Disabled {
		...
		if current := block.NumberU64(); current > triesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - axisdb.IdealBatchSize)
			}
	...
	if !reorg && externTd.Cmp(localTd) == 0 {
		...
		if !reorg && block.NumberU64() == currentBlock.NumberU64() {
			if block.NumberU64() < axisparam.XIP1() {
...
func (bc *BlockChain) insertChain(chain types.Blocks, local bool) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		if axisparam.XIP1() <= block.NumberU64() {
			stakeState := stake.NewStakeState(state)
