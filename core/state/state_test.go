...
func (s *StateSuite) TestDump(c *checker.C) {
	...
	obj1.AddBalance("axis", big.NewInt(22))
	obj2 := s.state.GetOrNewStateObject(toAddr([]byte{0x01, 0x02}))
	obj2.SetCode(crypto.Keccak256Hash([]byte{3, 3, 3, 3, 3, 3, 3}), []byte{3, 3, 3, 3, 3, 3, 3})
	obj3 := s.state.GetOrNewStateObject(toAddr([]byte{0x02}))
	obj3.SetBalance("axis", big.NewInt(44))
...
func TestSnapshot2(t *testing.T) {
	...
	so0.SetBalance("axis", big.NewInt(42))
	...
	so1.SetBalance("axis", big.NewInt(52))
...
func compareStateObjects(so0, so1 *stateObject, t *testing.T) {
	...
	if so0.Balance("axis").Cmp(so1.Balance("axis")) != 0 {
		t.Fatalf("Balance mismatch: have %v, want %v", so0.Balance("axis"), so1.Balance("axis"))
	}
