...
func (self *StateDB) GetAxisFee(contractAddr *common.Address, tfee *assets.Token) (sfee utils.U256, e error) {
...
func (self *StateDB) GetAxisGasLimit(to *common.Address, tfee *assets.Token, gasPrice *big.Int) (gaslimit uint64, e error) {
	if gasPrice == nil || gasPrice.Sign() <= 0 {
		e = errors.New("gas must > 0")
		return
	}
	if axis_fee, err := self.GetAxisFee(to, tfee); err != nil {
		e = err
		return
	} else {
		gaslimit = big.NewInt(0).Div(axis_fee.ToInt(), gasPrice).Uint64()
		return
	}
...
func (self *StateDB) GetTxGasLimit(tx *types.Transaction) (gaslimit uint64, e error) {
	if gaslimit, e = self.GetAxisGasLimit(tx.To(), &tx.Stxt().Fee, tx.GasPrice()); e != nil {
		return
	} else {
		return
	}
...
func (self *StateDB) Copy() *StateDB {
	...
	for addr := range self.journal.dirties {
		// As documented [here](https://github.com/axis-cash/go-axis/pull/16485#issuecomment-380438527),
