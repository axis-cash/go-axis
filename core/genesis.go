...
func SetupGenesisBlock(db axisdb.Database, genesis *Genesis) (*params.ChainConfig, common.Hash, error) {
	...
	if (stored == common.Hash{}) {
		if genesis == nil {
			...
			genesisConfig, err := base64.StdEncoding.DecodeString(params.GenesisConfig)
			if err != nil {
			   fmt.Errorf("invalid genesis config: %v", err)
			}
			genesis_tmp := new(Genesis)
			err = json.Unmarshal(genesisConfig, genesis_tmp)
			if err != nil {
			   fmt.Errorf("invalid genesis file: %v", err)
			}
			//bytes, _ := genesis.MarshalJSON()
			genesis = genesis_tmp;
			//log.Info("byte:"+string(bytes))
...
func (g *Genesis) ToBlock(db axisdb.Database) *types.Block {
	if db == nil {
		db = axisdb.NewMemDatabase()
	}
	statedb, _ := state.New(state.NewDatabase(db), nil)
	statedb.RegisterToken(state.EmptyAddress, "AXIS")
	statedb.AddBalance(state.EmptyAddress, "AXIS", new(big.Int).Mul(big.NewInt(250000000), big.NewInt(1e+18)))

	axis := common.BytesToHash(common.LeftPadBytes([]byte("AXIS"), 32))
	...
	for _, addr := range keys {
		...
		if addr == state.EmptyAddress {
			...
			if account.Balance != nil && account.Balance.Sign() > 0 {
				statedb.AddBalance(addr, "AXIS", account.Balance)
			}
			statedb.SetCode(addr, account.Code)
		} else {
			if account.Balance != nil && account.Balance.Sign() > 0 {
				asset := assets.Asset{Tkn: &assets.Token{
					Currency: *axis.HashToUint256(),
...
func (g *Genesis) Commit(db axisdb.Database) (*types.Block, error) {
	block := g.ToBlock(db)
...
// The block is committed as the canonical head block.
func (g *Genesis) MustCommit(db axisdb.Database) *types.Block {
...
// DeveloperGenesisBlock returns the 'gaxis --dev' genesis block. Note, this must
