...
func (st *StateTransition) preCheck() error {
	...
	if curency != "AXIS" {
		...
		if st.state.GetBalance(*to, "AXIS").Cmp(taval) < 0 {
			return errInsufficientBalanceForGas
		}
		st.state.AddBalance(*to, curency, st.msg.Fee().Value.ToRef().ToRef().ToIntRef())
		st.state.SubBalance(*to, "AXIS", taval)
...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	if contractCreation {
		...
		if evm.BlockNumber.Uint64() < axisparam.XIP1() || (to != nil && st.state.IsContract(*to)) {
...
func (st *StateTransition) refundGas() {
	...
	// Return AXIS for remaining gas, exchanged at the original rate.
	remaining := new(big.Int).Mul(new(big.Int).SetUint64(st.gas), st.gasPrice)

	if remaining.Sign() > 0 {
		curency := strings.ToUpper(common.BytesToString(st.msg.Fee().Currency.NewRef()[:]))
		if curency != "AXIS" {
			st.state.AddBalance(*st.msg.To(), "AXIS", remaining)
