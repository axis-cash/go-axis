...
import (
	...
	"github.com/axis-cash/go-axis/zero/consensus"
...
func (self *StakeState) RecordVotes(batch axisdb.Batch, block *types.Block) error {
...
func BlockShareNum(getter axisdb.Getter, block common.Hash) (num uint64) {
...
func SeleteBlockShare(getter axisdb.Getter, block common.Hash) (idx []uint32, shares []*Share) {
...
func (self *StakeState) NeedTwoVote(num uint64) bool {
	...
	if num > axisparam.XIP1()+window_size {
...
func GetStakePoolByBlockNumber(getter axisdb.Getter, id common.Hash, blockHash common.Hash, blockNumber uint64) *StakePool {
...
func GetBlockRecords(getter axisdb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share, pools []*StakePool) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
...
func (self *StakeState) getBlockRecords(getter axisdb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share, pools []*StakePool, err error) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func GetShare(getter axisdb.Getter, hash common.Hash) *Share {
	ret := ShareDB.GetObject(getter, hash[:], &Share{})
	return ret.(*Share)
}

func GetShareByBlockNumber(getter axisdb.Getter, id common.Hash, blockHash common.Hash, blockNumber uint64) *Share {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func GetSharesByBlock(getter axisdb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func (self *StakeState) getShares(getter axisdb.Getter, blockHash common.Hash, blockNumber uint64) (shares []*Share, err error) {
	records := state.StakeDB.GetBlockRecords(getter, blockNumber, &blockHash)
	for _, record := range records {
		if record.Name == "share" {
			for _, each := range record.Pairs {
...
func (self *StakeState) ProcessBeforeApply(bc blockChain, header *types.Header) (err error) {
	...
	// if header.Number.Uint64() > axisparam.XIP1() && header.Number.Uint64()%100 == 0 {
	// 	NewTree(self, header.Number.Uint64()).Midtraverse()
	// }
...
func (self *StakeState) statisticsByWindow(header *types.Header, bc blockChain) error {
	...
	if header.Number.Uint64() < 1 || header.Number.Uint64()-1 < axisparam.XIP1() {
		return nil
	}
	...
	if preHeader.Number.Uint64() >= axisparam.XIP1()+statisticsMissWindow {
...
func (self *StakeState) processOutDate(header *types.Header, bc blockChain) (err error) {
	...
	if header.Number.Uint64() < outOfDatePeriod || header.Number.Uint64()-outOfDatePeriod < axisparam.XIP1() {
		return
	}

	preNumber := header.Number.Uint64() - outOfDatePeriod
	if preNumber < axisparam.XIP1() {
		return
	}
