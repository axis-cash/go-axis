...
import (
	...
	"github.com/axis-cash/go-axis/zero/txs/zstate/txstate/data_v1"
...
func (self *State) RecordState(putter axisdb.Putter, root *c_type.Uint256) {
...
func (self *State) PreGenerateRoot(header *types.Header, ch Chain) {
	if header.Number.Uint64() == (axisparam.SIP2()) {
		hash := header.ParentHash
		number := header.Number.Uint64() - 1
		size := number
