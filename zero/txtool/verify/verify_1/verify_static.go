...
import (
	...
	"github.com/axis-cash/go-axis/zero/txtool/verify/verify_utils"
)
...
func (self *verifyWithoutStateCtx) verifyCmd() (e error) {
	...
	if self.tx.Desc_Cmd.RegistPool != nil {
		if self.tx.Desc_Cmd.RegistPool.FeeRate > axisparam.HIGHEST_STAKING_NODE_FEE_RATE {
			e = verify_utils.ReportError(fmt.Sprintf("regist pool the fee rate must < %v%%", axisparam.HIGHEST_STAKING_NODE_FEE_RATE), self.tx)
			return
		}
		if self.tx.Desc_Cmd.RegistPool.FeeRate < axisparam.LOWEST_STAKING_NODE_FEE_RATE {
			e = verify_utils.ReportError(fmt.Sprintf("regist pool fee must >= %v%%", axisparam.LOWEST_STAKING_NODE_FEE_RATE/100), self.tx)
			return
		}
	}
...
func (self *verifyWithoutStateCtx) verifyBalance() (e error) {
	if self.num >= axisparam.XIP6() {
		...
		if self.oout_count > axisparam.MAX_O_OUT_LENGTH {
			e = verify_utils.ReportError("verify error: oout_size > 10", self.tx)
			return
		}
		if self.zout_count > axisparam.MAX_Z_OUT_LENGTH_OLD {
