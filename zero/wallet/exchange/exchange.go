...
func (self *Exchange) indexBlocks(batch axisdb.Batch, utxosMap map[PkKey][]Utxo, blockMap map[uint64]*BlockInfo, nils []c_type.Uint256) (delRoots []c_type.Uint256, err error) {
...
func (self *Exchange) getMergeUtxos(from *c_type.Uint512, currency string, zcount int, left int) (mu MergeUtxos, e error) {
	...
	for _, tkn := range ck.Tkns() {
		if utxos, r := self.findUtxos(from, utils.BytesToCurrency(tkn.Currency[:]), tkn.Value.ToInt()); r == nil || r.Sign() > 0 {
			e = errors.New("No enough AXIS coins for fee")
			return
		} else {
...
func (self *Exchange) GenMergeTx(mp *MergeParam) (txParam *txtool.GTxParam, e error) {
	...
	ck := assets.NewCKState(false, &assets.Token{utils.CurrencyToUint256("AXIS"), utils.U256(*default_fee_value)})
...
func (self *Exchange) merge() {
	...
	self.accounts.Range(func(key, value interface{}) bool {
		account := value.(*Account)
		if count, txhash, err := self.Merge(account.pk, "AXIS", false); err != nil {
