...
makeAccount(){
    ...
    echo "personal.newAccount()" > /tmp/gaxis-input
    echo "${PASSWORD}" > /tmp/gaxis-input
    echo "${PASSWORD}" > /tmp/gaxis-input
    while read lineA; do
        if [ -z ${lineA} ]; then
            echo "finished" >&2
        ...
        if read lineA; then
            ...
            if [ $length == 1 -a "${LINEA_NO_WHITESPACE}" == ">" ]  ; then
             #    echo "it is a hint" >&2
                ...
                echo "meet fatal error, please check the log in ~/working/gaxis.log" >&2
                exit -1
            fi
            ...
            if [ "$lineB" == "BEGIN_PASSWORD:true" ]; then
              ...
              #  echo "${PASSWORD}"
                ...
                if [ $length -le 10 ]; then
                    ...
                    printf '%s\n' "personal.newAccount()" >/tmp/gaxis-input
                    sleep 5
                    continue 
                fi 
                ...
                printf 'miner.setAxisbase("%s")\n' "${ACCOUNT}" 
                printf 'miner.setAxisbase("%s")\n' "${ACCOUNT}" >/tmp/gaxis-input &
                echo 'miner.start()' >/tmp/gaxis-input &
                exit 
...
if [ "${result}" == "-1" ]; then
    ...
    set -o xtrace
    nohup ./bin/gaxis --alpha --datadir=${DATADIR} --rpc --rpcport ${RPCPORT}  --port ${SERVERPORT} --rpccorsdomain "*" &> ~/.log/gaxis.log &
    set +x
    sleep 30
fi
result=$(checkProcessWorking "$PATTERN_MAIN_PROCESS")
if [ "${result}" == "-1" ]; then
    echo "Please check why the main process of gaxis not running with log in ~/working/"
    exit
fi 
if [ -p "/tmp/gaxis-input" ] || [ -f "/tmp/gaxis-input" ]; then
    rm /tmp/gaxis-input
fi
if [ -p "/tmp/gaxis-output" ] || [ -f "/tmp/gaxis-output" ]; then
    rm /tmp/gaxis-output
fi
if [ -p "./localio" ] || [ -f "./localio" ]; then
    rm ./localio
fi
mkfifo -Z --mode='a=rwx' /tmp/gaxis-input
mkfifo -Z --mode='a=rwx' /tmp/gaxis-output
mkfifo -Z --mode='a=rwx' ./localio
cat /tmp/gaxis-input|./bin/gaxis --datadir=~/.datadir attach > /tmp/gaxis-output &
exec 9<> /tmp/gaxis-input
readingFromOutput &
backgroundPid=$!
printf '%s\n' 'axis.accounts' >/tmp/gaxis-input
