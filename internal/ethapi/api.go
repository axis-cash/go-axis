...
import (
	...
	"github.com/axis-cash/go-axis/zero/txs"
	...
	ztx "github.com/axis-cash/go-axis/zero/txs/tx"
...
func (s *PrivateAccountAPI) ImportRawKey(privkey string, password string, v *int, a *uint64) (address.PKAddress, error) {
	...
	if maxBlockNumber < axisparam.XIP0() {
		if version == 2 {
			return address.PKAddress{}, errors.New(fmt.Sprintf("account version is 2 must be after XIP5=%v", axisparam.XIP0()))
		}
	}
...
type CallArgs struct {
	...
	GasCurrency Smbol                     `json:"gasCy"` //default AXIS
...
func (s *PublicBlockChainAPI) doCall(ctx context.Context, args CallArgs, blockNr rpc.BlockNumber, vmCfg vm.Config, timeout time.Duration) ([]byte, uint64, bool, error) {
	...
	if args.To != nil && state.IsContract(common.BytesToAddress(args.To[:])) && args.GasCurrency.IsNotAxis() {
		m, d := state.GetTokenRate(common.BytesToAddress(args.To[:]), string(args.GasCurrency))
		if m.Sign() == 0 || d.Sign() == 0 {
			return nil, 0, false, errors.New("gasCurrency must be AXIS or nil")
		}
		state.AddBalance(common.BytesToAddress(args.To[:]), "AXIS", fee)
...
func (s *PublicBlockChainAPI) GetDecimal(ctx context.Context, tokenName string) (*hexutil.Uint, error) {
	...
	if tokenName == "" {
		...
		if tokenName == "axis" || tokenName == "AXIS" {
			return nil, errors.New("tokenName can not be axis!")
...
func (args *SendTxArgs) setDefaults(ctx context.Context, b Backend) error {
	...
	if args.To == nil || !state.IsContract(common.BytesToAddress(args.To.Bytes())) {
		if args.GasCurrency.IsNotEmpty() && args.GasCurrency.IsNotAxis() {
			return errors.New(`GasCurrency must be null or AXIS`)
		}
	} else {
		if args.GasCurrency.IsNotAxis() {
...
func (s *PublicTransactionPoolAPI) CreatePkg(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	...
	if args.GasCurrency.IsNotAxis() {
		return common.Hash{}, errors.New("create pkg gasCurrency must be axis")
	}
