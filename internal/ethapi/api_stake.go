...
func (args *RegistStakePoolTxArg) setDefaults(ctx context.Context, b Backend) error {
	...
	if uint32(*args.Fee) < axisparam.LOWEST_STAKING_NODE_FEE_RATE {
		return errors.New(fmt.Sprintf("fee rate can not less then %v", axisparam.LOWEST_STAKING_NODE_FEE_RATE))
	}
	if uint32(*args.Fee) > axisparam.HIGHEST_STAKING_NODE_FEE_RATE {
		return errors.New(fmt.Sprintf("fee rate can not large then  %v", axisparam.HIGHEST_STAKING_NODE_FEE_RATE))
	}
...
func (s *PublicStakeApI) ModifyStakePoolFee(ctx context.Context, from address.MixBase58Adrress, fee hexutil.Uint64) (common.Hash, error) {
	...
	if uint32(fee) < axisparam.LOWEST_STAKING_NODE_FEE_RATE {
		return common.Hash{}, errors.New(fmt.Sprintf("fee rate can not less then %v", axisparam.LOWEST_STAKING_NODE_FEE_RATE))
	}
	if uint32(fee) > axisparam.HIGHEST_STAKING_NODE_FEE_RATE {
		return common.Hash{}, errors.New(fmt.Sprintf("fee rate can not large then  %v", axisparam.HIGHEST_STAKING_NODE_FEE_RATE))
	}
...
func newRPCStatisticsShare(mg *accounts.Manager, shares []*stake.Share, api *PublicStakeApI, ctx context.Context) []map[string]interface{} {
	...
	for _, share := range shares {
		...
		if share.LastPayTime > share.BlockNumber+0 {
			continue
		}
