...
func (n *Node) OpenDatabase(name string, cache, handles int) (axisdb.Database, error) {
	if n.config.DataDir == "" {
		return axisdb.NewMemDatabase(), nil
	}
	return axisdb.NewLDBDatabase(n.config.ResolvePath(name), cache, handles)
}
