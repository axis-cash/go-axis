...
var (
	...
	//teamAddress = common.BytesToAddress( []byte("3ZNBvanMkxktSb6kcg4TynF5bHDw4ProHwB3gUjAvLrx2XJUu23d6zAjaKSCWCkTBqZqYqb8PNEsvSFMqZovBSbD"))
	communityAddress = common.Base58ToAddress("8unZXT72FhLfNKKjoTp9ar7TVH4iVtGXjoSf8y21Kxq7pbdjjPf5UT2ZHz6SS47mME7w4raBNdK5jMPRxp5AzyAwPdg4rAo2sicS9o6yZde6FYCo1FiRu88P2ohEuVshpbT")
)
...
func accumulateRewards(config *params.ChainConfig, statedb *state.StateDB, header *types.Header, gasReward uint64) {
	...
	if axisparam.Is_Dev() {
		reward = new(big.Int).Set(oneAxis)
	}
...
func accumulateRewardsV1(config *params.ChainConfig, statedb *state.StateDB, header *types.Header) *big.Int {
	poolBalance := statedb.GetBalance(state.EmptyAddress, "AXIS")
	...
	statedb.SubBalance(state.EmptyAddress, "AXIS", reward)
	return reward
}
...
func accumulateRewardsV2(statedb *state.StateDB, header *types.Header) *big.Int {
	...
	statedb.AddBalance(communityRewardPool, "AXIS", new(big.Int).Div(rewardStd, big.NewInt(15)))
	statedb.AddBalance(teamRewardPool, "AXIS", new(big.Int).Div(new(big.Int).Mul(reward, big2), big.NewInt(15)))
	...
	if header.Number.Uint64()%5 == 0 {
		...
		balance = statedb.GetBalance(communityRewardPool, "AXIS")
		statedb.SubBalance(communityRewardPool, "AXIS", balance)
...
func accumulateRewardsV3(statedb *state.StateDB, header *types.Header) *big.Int {
	...
	statedb.AddBalance(teamRewardPool, "AXIS", new(big.Int).Div(reward, big.NewInt(5)))

	if header.Number.Uint64()%500 == 0 {
		balance := statedb.GetBalance(teamRewardPool, "AXIS")
		statedb.SubBalance(teamRewardPool, "AXIS", balance)
		assetTeam := assets.Asset{Tkn: &assets.Token{
			Currency: *common.BytesToHash(common.LeftPadBytes([]byte("AXIS"), 32)).HashToUint256(),
